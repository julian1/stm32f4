...
{
  assert(end - start > 0); // needed for null terminal
  a->start = start;
  a->end   = end;
...
int32_t cstring_back( const cstring_t *a)
{
  ...
  // this is crappy... not sure desirable
  return *(a->pos - 2);
}
...
int main()
{
  ...
  assert(cstring_size( &s) == 1);
  std::cout << "count " << cstring_size( &s) << "   string " << cstring_ptr( &s) << std::endl;
  ...
  std::cout << "last '" << ((char ) cstring_back( &s) ) << "'" << std::endl;
  std::cout << "first " << ((char) cstring_front( &s) )<< std::endl;
  ...
  cstring_push(&s, 'h' );
  assert( cstring_back( &s) == 'h' );
  assert( cstring_front( &s) == 'h' );
