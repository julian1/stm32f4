...
void ranging_init( ranging_t *ranging, _mode_t *mode)
{
  ...
  memset( ranging, 0, sizeof(ranging_t));


  // review. declared extern
  // not clear required in app_t, can limit scope to decode_t

  ranging->magic            = RANGING_MAGIC;

 ranging->mode                = mode;

 ranging->ranges             = range_init_values;
 ranging->ranges_sz          = range_init_sz;     // issue with const correctness
 ranging->range_idx          = 0;
...
//
bool ranging_range_dir_valid( ranging_t *ranging, uint32_t range_idx, bool dir);


void ranging_range_switch( ranging_t *ranging, uint32_t range_idx);





// may want to be static
...
bool ranging_range_dir_valid( ranging_t *ranging, uint32_t range_idx, bool dir)    // 1 up. 0 down
{
