...
static bool spi_repl_reg_query( spi_t *spi, const char *cmd, environment_t *environment_ )
{
  ...
  */
	...
	assert( spi);
	assert( environment_);
	assert( environment_->line_freq == 50);
...
*/
  ...
  // adc
    ...
    aperture_print( aperture,  environment_->line_freq);
  }
...
bool app_repl_statement( app_t *app,  const char *cmd)
{
  ...
  */
  ...
  assert( app->environment->line_freq == 50);
