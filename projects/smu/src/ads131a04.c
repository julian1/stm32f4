...
static uint8_t adc_read_register(uint32_t spi, uint8_t r )
{
  ...
  */
  ...
  if(val >> 8 != j) {
    printf("bad acknowledgement address\n");
...
static uint8_t adc_write_register(uint32_t spi, uint8_t r, uint8_t val )
{
  ...
  */
  ...
  if(ret != ((1u << 5 | r) << 8 | val)) {
    printf("bad write acknowledgement address or value\n");
    // need something better....
    return -1;
  }
...
int adc_reset( uint32_t spi, uint8_t reg)
{
  ...
  */
  ...
  ////////////
  // reset
  printf("adc assert reset\n");
  ice40_reg_clear(spi, reg, ADC_RST);

  printf("adc before msleep\n");
  msleep(20);
  printf("adc after msleep\n");
...
int adc_init( uint32_t spi, uint8_t reg)
{
  ...
  if(a_sys_cfg != 0x60) {
    printf("a_sys_cfg not expected default\n");
    return -1;
  } else {
    printf("a_sys_cfg val ok\n");
  }
  ...
  // printf("d_sys_cfg %02x\n", d_sys_cfg);
  printf("d_sys_cfg %s\n", format_bits(buf, 8, d_sys_cfg));
  if(d_sys_cfg != 0x3c) {
    printf("d_sys_cfg not expected default\n");
    return -1;
  }
...
*/
  ...
  if(clk1 != 0x08) {
    printf("clk1 not expected default\n");
    return -1;
  }
...
#define CLK_DIV_VAL    5      // 101 : fICLK = fCLKIN / 10
  ...
  if(clk2 != 0x86) {
    printf("clk2 not expected default\n");
    return -1;
  }
