...
int agg_test2()
{
  ...
  // seem to have the italic text.
    ...
    typedef agg::serialized_integer_path_adaptor<short int, 6>  path_type ;
    path_type  m_path(x, 252, 0, 0, 1.0) ; 

    // Needs to be fixed in the constructor. or the private member made public so we can set.
    // m_path.m_scale = 1.0 ;
#endif
...
// hmmmm. doesn't work.
    ...
    // we need other letters 'j' with hanging to
      ...
      // whoot it links
      assert( arial_glyph['b'] );
     path_type &path = *arial_glyph['b'];


    agg::conv_transform<path_type > trans(path, mtx);
    agg::conv_curve<agg::conv_transform<  path_type> > curve(trans);
