...
void exti0_isr(void)
{
  ...
  if (exti_direction == FALLING) {
    ...
    uint32_t x1 = count - oc_value;
    uint32_t x2 = period - count;

    uint32_t down =  x1 + x2;
    int32_t adjust = (down / 2) - x1;

    usart_printf("  x1 %u, x2 %u, down %u, adjust %d\n", x1, x2, down, adjust );

    if(adjust < 100000) { 

      oc_value += adjust;
   
      // problem if it's not near the value
      // usart_printf("  oc_value now    %u\n", oc_value  );
