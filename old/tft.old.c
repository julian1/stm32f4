...
static inline void tft_write_cmd( tft_t *tft, uint16_t cmd)
{
  ...
  if(first) {
    char buf[101];
    printf("(tft->fmc_addr ) %s cmd\n", str_format_bits( buf, 100, tft->fmc_addr  ));
    first = false;
  }
...
static inline void tft_write_data( tft_t *tft, uint16_t data)
{
  ...
  if(first) {
    char buf[101];
    printf("(tft->fmc_addr ) %s data w\n", str_format_bits( buf, 100, (tft->fmc_addr | tft->fmc_cd))  );
    first = false;
  }
...
/*
  This all looks correct the 19th bit is asserted...
  Perhaps  there is a fundamental issue that the address is not asserted at the same time as the CS

  Or we should assert CS.  the correct address line.
  or we are not isolating the address problem.

  eg. it works OK. for VFD selection.
